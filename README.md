...
# بازآرایی پیشرفته با استفاده از Dependency Injection و مدیریت پیکربندی

## هدف

حذف وابستگی‌های مستقیم به مقادیر حساس و پیاده‌سازی‌های خاص با استفاده از تزریق وابستگی و ماژول مدیریت پیکربندی
...
- دریافت API Key و endpoint از منابع امن  
- متدهایی برای خواندن پیکربندی‌ها  

### Dependency Injection
- نمونه‌سازی درگاه‌ها با استفاده از پارامترهای ورودی  
- حذف hardcode  

### PaymentProcessor
- دریافت شیء PaymentGateway به صورت abstraction  

### PaymentGatewayDemo (Main)
- خواندن پیکربندی از ConfigurationManager  
- ساخت درگاه با تزریق پارامتر  
- اجرای process و refund  
- نمایش توانایی استفاده از چندریختی  

---

## مزایا

| ویژگی                 | قبل از بازآرایی             | بعد از بازآرایی                         |
|-----------------------|------------------------------|-----------------------------------------|
| مدیریت پیکربندی       | hard-coded در کلاس‌ها        | جداشده در ConfigurationManager         |
| وابستگی‌ها            | به پیاده‌سازی خاص            | به Interface (polymorphic)              |
| افزودن درگاه جدید     | نیاز به تغییر کد اصلی        | بدون تغییر در منطق مرکزی               |
| تست‌پذیری             | ضعیف                         | بالا (با استفاده از mock interface)    |

## جمع‌بندی

اکنون سیستم پرداخت ما از معماری‌ای بهره می‌برد که مطابق با اصول SOLID، ایمن، تست‌پذیر و توسعه‌پذیر است.

---

### توضیح نهایی

ما این آزمایش را در چهار بخش پیش برده‌ایم.
در این چهار مرحله، تلاش شد تا ساختار سیستم پرداخت به صورت اصولی و شی‌ءگرا بازطراحی شود.
در مرحله اول، با شناسایی کد اسمل‌ها و نقض اصول SOLID، مشکلات موجود بررسی گردید.
در مرحله دوم، منطق پردازش پرداخت با استفاده از وراثت و چندریختی از هم تفکیک شد.
در مرحله سوم، یک لایه انتزاعی برای درگاه‌های پرداخت طراحی شد تا سیستم انعطاف‌پذیرتر گردد.
در مرحله چهارم، با استفاده از Dependency Injection و ماژول مدیریت پیکربندی، وابستگی‌ها کاهش یافته و امنیت و تست‌پذیری ارتقاء یافت.
در نهایت، سیستم به گونه‌ای طراحی شد که قابل توسعه، نگه‌داری و تست آسان باشد.

همچنین در نهایت فایل Main.java نیز اضافه شد تا بررسی و تست درستی کد و کارکرد آن صحت‌سنجی بشود.

---

### تقسیم کار
در این آزمایش تلاش شد تا تسک‌ها به طور یکسان بین اعضای تیم پخش شود.
مرحله اول مطابق کانبان برد، توسط هر 3 نفر تحلیل و نهایی شد و توسط یک نفر کامیت شد. بعد از آن سه مرحله باقی‌مانده بین سه نفر دیگر تقسیم شد و مشابه همان کامیت‌ها اعمال شدند. تنها استثنا در بخش دوم برای جاواداک بود که به دلیل برخی دلایل یکی دیگر از  اعضا مسئولیت آن را به عهده گرفت.

[لینک کانبان‌برد](https://github.com/users/Amirreza81/projects/2)
